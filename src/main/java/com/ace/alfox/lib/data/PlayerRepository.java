...
public class PlayerRepository extends ObjectRepositoryFacade<Player> {
  ...
  /**
   * signUp functions takes a username and password, guaranteed that username is unique due to how the controller handles it (baring some kind of timing issue?)
   *
   * It generates a salt via SecureRandom, and then generates a hashed password based on the password supplied, and the salt.
   * @param username
   * @param password
   * @return
   */
  public Player signUp(String username, String password){
    ...
    super.insert(newPlayer);//Make sure to do some check here to make sure it's inserted! Evan was too vague.
    return newPlayer;

  }

  /**
   * generateHash takes MessageDigest and uses SHA-256 to generate hashes of the user salt and password, 10000 times.
   * @param password
   * @param salt
   * @return
   */
